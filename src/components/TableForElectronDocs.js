 ...
 if(pagBUTTON){
    ...
    //   },
      {
        ...
        muiEditTextFieldProps: {
          required: true,
          error: !!validationErrors?.docs,
          helperText: validationErrors?.docs,
  ...
  const table = useMaterialReactTable({ 
    ...
    renderCreateRowDialogContent: ({ table, row, internalEditComponents }) => (
      <>
        <DialogTitle variant="h5">Yeni sənəd növü əlavə edin</DialogTitle>
        <DialogContent
...
<path d="M10.668 4.5L12.8017 6.75M9.24561 15H14.9353" stroke="#4B7D83" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
    ...
    renderTopToolbarCustomActions: ({ table }) => (
      ...
      >
      Yeni elektron sənəd növü əlavə edin
      </Button>
    ),
...
function validateUser(user) {
  return {
    docs: !validateRequired(user.docs)
