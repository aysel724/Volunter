 ...
 if(pagBUTTON){
  ...
  const columns = useMemo(
    () => [
      ...
      // },
      {
        ...
        header: 'Dil səviyyəsi',
        muiEditTextFieldProps: {
          required: true,
          error: !!validationErrors?.languageLevel,
          helperText: validationErrors?.languageLevel,
  ...
  const table = useMaterialReactTable({ 
    ...
    renderCreateRowDialogContent: ({ table, row, internalEditComponents }) => (
      <>
        <DialogTitle variant="h5">Yeni  dil biliyi səviyyəsi əlavə edin</DialogTitle>
        <DialogContent
...
function validateUser(user) {
  return {
    languageLevel: !validateRequired(user.languageLevel)
