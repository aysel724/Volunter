...
import { fakeData4, usStates } from '../makeData';
 ...
 if(pagBUTTON){
  ...
  const table = useMaterialReactTable({ 
    ...
    renderCreateRowDialogContent: ({ table, row, internalEditComponents }) => (
      <>
        <DialogTitle variant="h5">Yeni vətəndaşlıq əlavə edin</DialogTitle>
        <DialogContent
...
function useGetUsers() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      await new Promise((resolve) => setTimeout(resolve, 1000)); //fake api call
      return Promise.resolve(fakeData4);
    },
